...
grep -q $'K_xi\ttpf_4d_xi_motion_readout_scale' "$OUT/right_TPFCore/run_info.txt"
grep -q $'acceleration_formula\ta=-K_xi\*Xi_eff_spatial' "$OUT/right_TPFCore/run_info.txt"
grep -q $'xi_kernel_factor_mode\t' "$OUT/right_TPFCore/run_info.txt"
! grep -q 'active_for_this_route' "$OUT/right_TPFCore/run_info.txt"

grep -q '"tpf_core_law_mode": "xi_kernel_deformed"' "$OUT/right_TPFCore/render_manifest.json"
grep -q '"K_xi": "tpf_4d_xi_motion_readout_scale"' "$OUT/right_TPFCore/render_manifest.json"
grep -q '"acceleration_formula": "a=-K_xi\*Xi_eff_spatial"' "$OUT/right_TPFCore/render_manifest.json"
grep -q '"xi_kernel_factor_mode": ' "$OUT/right_TPFCore/render_manifest.json"
