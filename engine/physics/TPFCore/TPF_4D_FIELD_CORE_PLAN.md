...
- Stage 5 (this change): add richer view-plane CSV exports, source marker CSV, and optional post-process plotting script wiring behind `--plot`; visualization/export inspection artifacts only (no residual/source-field math changes, no dynamics/source-worldline/time-evolution claims, no proof/validation claims).
...
- Stage 4/5 benchmark harness is inspection-only and does not claim validation/proof.
