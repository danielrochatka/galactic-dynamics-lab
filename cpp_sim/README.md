...
**Display units** (axis labels, animation captions, overlays) are controlled by **`display_distance_unit`**, **`display_time_unit`**, **`display_velocity_unit`**, **`display_units_in_overlay`**, and **`display_show_unit_reference`** in the run config (written to **`run_info.txt`**). Simulation and snapshot CSVs remain **SI**; see **[../docs/UNITS.md](../docs/UNITS.md)**.
